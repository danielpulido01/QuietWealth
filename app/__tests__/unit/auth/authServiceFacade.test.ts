...
describe("authServiceFacade", () => {
  ...
  it("delegates login to authService", async () => {
    ...
    await expect(
      authServiceFacade.login({ email: "user@example.com", password: "secret" }),
  ...
  it("formats fallback messages for unknown errors", () => {
    expect(authServiceFacade.toErrorMessage(new Error("Specific failure"))).toBe(
      "Specific failure",
    );
  ...
  it("delegates the remaining auth operations to authService", async () => {
    ...
    await authServiceFacade.requestPasswordReset(
      "user@example.com",
      "https://quietwealth.example/reset",
    );
    ...
    expect(resetRequestSpy).toHaveBeenCalledWith(
      "user@example.com",
      "https://quietwealth.example/reset",
    );
